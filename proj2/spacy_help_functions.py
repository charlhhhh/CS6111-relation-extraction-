...
def extract_relations(doc, spanbert, rel, entities_of_interest, conf, stats, X_confidence):
    ...
    for sentence in doc.sents:
        ...
        for ep in entity_pairs:
            ...
            if (ep[2][1] == entities_of_interest[0] and ep[1][1] in entities_of_interest[1:]):
        ...
        for ex, pred in list(zip(examples, preds)):
            ...
            if relation == rel:
                ...
                obj = ex["obj"][0]
                confidence = pred[1]
                existing_confidence = X_confidence.get((subj, relation, obj), 0)
                print("\t\tRelation: {} (Confidence: {:.3f})\nSubject: {}\tObject: {}".format(relation, confidence, subj, obj))
                if confidence > conf:
                    if res[(subj, relation, obj)] < confidence and existing_confidence < confidence:
